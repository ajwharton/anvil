...
- **Agent-first control**: Anvil owns MCP/tools + optional harness + portable prompts; **you bring the agent model** (frontier or local). Usable solo; shines when an agent watches cliffs and switches methods live

Product thesis: [`docs/product.md`](docs/product.md) · agent split: [`docs/agentic-control.md`](docs/agentic-control.md) · prompts: [`prompts/agent/`](prompts/agent/). Not a full MoE train stack on day one. Inspiration one-liner lives in `README.md` only.
...
| `docs/agentic-control.md` | Anvil harness/MCP vs user brain; portable prompt pack |
| `prompts/agent/` | Generic operator prompts for Anvil or foreign harnesses |
