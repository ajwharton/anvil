...
Modes
-----
* **local** (default): run a short SFT/robot_offline job, classify
  ``metrics.jsonl``, append ``decisions.jsonl``, optionally export.
* **control**: if ``ANVIL_CONTROL_URL`` is up, list runs / observe / pause via
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.job == "robot_offline":
        ...
        res = run_robot_offline(
            ...
            fetch_remote=False,
            early_stop=False,
            export_dir=str(run_dir / "export") if args.export else None,
        )
    ...
    if decision.action == ActKind.EXPORT and args.export:
        acted.append(f"export requested path={run_dir / 'export'}")
    elif decision.action == ActKind.WAIT:
        acted.append("wait — keep training next cycle")
    elif decision.action == ActKind.LOWER_LR:
        acted.append(f"recommend knobs_patch={decision.knobs_patch}")
    elif decision.action in {ActKind.PAUSE, ActKind.STOP}:
        acted.append(f"{decision.action.value} recommended")
    else:
        acted.append(decision.action.value)
    ...
    if decision.classification.value == "broken" and not losses:
        return 2
    return 0
